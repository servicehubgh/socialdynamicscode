...
function onBtnRecordClicked (){
	if (localStream == null) {
		...
		mediaRecorder.onstop = function(){
			...
			video_blob = new Blob(chunks, {type: "video/webm"});
			chunks = [];

			var videoURL = window.URL.createObjectURL(video_blob);
			...
			video_name  = "video_"+rand+".webm" ;

			downloadLink.setAttribute( "download", video_name);
			downloadLink.setAttribute( "name", video_name);
		};
...
async function onStateClicked(){

    let formData = new FormData();

    formData.append("video", video_blob);
    formData.append("info", video_name);

    fetch('/record_video', {method: "POST", body: formData}).then(response => response.json())
	.then(data => {
